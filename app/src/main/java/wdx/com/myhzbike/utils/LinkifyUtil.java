...
public class LinkifyUtil {
    ...
    public static final ArrayList<String> backLinks(CharSequence t, int mask){
        if (t instanceof Spannable) {
        	ArrayList<LinkSpec> linkSpecs = doLinks((Spannable)t, mask);
        	ArrayList<String> urlString = new ArrayList<String>();
        	for (int i = 0; i < linkSpecs.size(); i++) {
        		urlString.add(linkSpecs.get(i).url);
			}
        	return urlString;
        } else {
        	SpannableString s = SpannableString.valueOf(t);
        	ArrayList<LinkSpec> linkSpecs = doLinks(s, mask);
    		ArrayList<String> urlString = new ArrayList<String>();
    		for (int i = 0; i < linkSpecs.size(); i++) {
    			urlString.add(linkSpecs.get(i).url);
    		}
    ...
    private static ArrayList<LinkSpec> doLinks(Spannable text, int mask){
     ...
     */
    public static final boolean addLinks(Spannable text, int mask, boolean isHaveUnderLine) {
        if (mask == 0) {
            return false;
        }
        ...
        for (LinkSpec link: links) {
            applyLink(link.url, link.start, link.end, text, isHaveUnderLine);
        }
     ...
     *  LinkMovementMethod.
     */
    public static final boolean addLinks(TextView text, int mask, boolean isHaveUnderLine) {
        if (mask == 0) {
            return false;
        }
        ...
        if (t instanceof Spannable) {
            if (addLinks((Spannable) t, mask, isHaveUnderLine)) {
            ...
            if (addLinks(s, mask, isHaveUnderLine)) {
                addLinkMovementMethod(text);
                text.setText(s);
...
//
//        if ((m == null) || !(m instanceof LinkMovementMethodUtil)) {
//            if (t.getLinksClickable()) {
//                t.setMovementMethod(LinkMovementMethodUtil.getInstance());
//            }
//        }
    }
    ...
    public static final boolean addLinks(Spannable s, Pattern p,
        ...
        while (m.find()) {
            ...
            if (allowed) {
                ...
                applyLink(url, start, end, s, true);
                hasMatches = true;
            }
        }
    ...
    private static final void applyLink(String url, int start, int end, Spannable text, boolean isHaveUnderLine) {
        URLSpanNoUnderline span = new URLSpanNoUnderline(url , isHaveUnderLine);

        text.setSpan(span, start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    }

    private static class URLSpanNoUnderline extends URLSpan {
        boolean isHaveUnderLine = true;
        public URLSpanNoUnderline(String url, boolean isHaveUnderLine) {
            super(url);
            this.isHaveUnderLine = isHaveUnderLine;
        }
        @Override public void updateDrawState(TextPaint ds) {
            super.updateDrawState(ds);
            ds.setUnderlineText(isHaveUnderLine);
        }
    }
